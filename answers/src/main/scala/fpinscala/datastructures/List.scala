...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  */
  ...
  // chapter. (We'll revisit this in a later chapter, when we discuss
  // laziness).
  ...
  // type being instantiated to `B => B`, then calling the built up
  // function with the `z` argument. Try expanding the definitions by
  // substituting equals for equals using a simple example, like
  // `foldLeft(List(1,2,3), 0)(_ + _)` if this isn't clear. Note
  // these implementations are more of theoretical interest - they
  // aren't stack-safe and won't work for large lists.
